...
int RuntimeObjectSystem::TestBuildAllRuntimeSourceFiles(  RCCppTestBuildFailCallback failCallback )
{
    ...
    {
        failCallbackLocal( NULL, TESTBUILDFAILTYPE_NO_FILES_TO_BUILD );
        if( m_pCompilerLogger ) { m_pCompilerLogger->LogInfo("No Runtime Source files to test\n"); }
    }
	...
	for( TFileList::iterator it = m_RuntimeFileList.begin(); it != m_RuntimeFileList.end(); ++it )
    {
        ...
        {
            int fileErrors = TestBuildFile( m_pCompilerLogger, this, file, failCallbackLocal );
            if( fileErrors < 0 )
            {
